...
# Prompt for OneDrive organization name
Write-Host ""
Write-ColoredOutput "PowerShell profile setup:" "Magenta"
Write-Host "1. Use OneDrive (enter organization name)"
Write-Host "2. Use local Documents folder"

$choice = Read-Host "Choose option (1 or 2)"

if ($choice -eq "1") {
    $oneDriveOrg = Read-Host "Enter your OneDrive organization name"
    $powerShellProfilePath = "${env:USERPROFILE}\OneDrive - ${oneDriveOrg}\Documents\PowerShell\Microsoft.PowerShell_profile.ps1"
} else {
    $powerShellProfilePath = "${env:USERPROFILE}\Documents\PowerShell\Microsoft.PowerShell_profile.ps1"
}
...
${symlinks} = @(
    @{Source="${dotfilesDir}\powershell\config.ps1"; Destination="${powerShellProfilePath}"},
	...
	@{Source="${dotfilesDir}\alacritty"; Destination="${env:APPDATA}\alacritty"},
    ...
    @{Source="${dotfilesDir}\posting"; Destination="${env:USERPROFILE}\.config\posting"},
    @{Source="${dotfilesDir}\ohmyposh\catppuccin.yaml"; Destination="${env:USERPROFILE}\Documents\PowerShell\catppuccin.omp.yaml"},
    @{Source="${dotfilesDir}\kanata\config.kbd"; Destination="${env:USERPROFILE}\kanata.kbd"},
    ...
    @{Source="${dotfilesDir}\yasb"; Destination="${env:USERPROFILE}\.config\yasb"},
